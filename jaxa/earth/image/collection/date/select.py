...
def select_multiple_dates_url(input, date):
    ...
    stac_date_url_all = get_all_children(
        input._session,
        "child",
        input.stac_collection.url[0][0],
        input.stac_collection.json[0][0],
    )

    # Detect STAC Date format and depth
    date_format_raw = input.stac_collection.json[0][0]["summaries"][
        "je:stac_date_format"
    ]
    ...
    stac_date_url, stac_date_id = select_date(
        input._session, stac_date_url_all, date_format, date
    )
...
def select_date(session, stac_date_url_all, DATE_FORMAT, date_lim_str):
    ...
    try:
        date_lim = [
            datetime.datetime.strptime(date_lim_str[i], "%Y-%m-%dT%H:%M:%S")
            for i in range(len(date_lim_str))
        ]
    except:
    ...
    if DATE_FORMAT == "YYYY-MM/DD":
        ...
        date_lim_tmp = [
            date_lim[0].replace(day=1),
            date_lim[1].replace(
                day=calendar.monthrange(date_lim[1].year, date_lim[1].month)[1]
            ),
        ]
        ...
        STR_FORMAT0 = "%Y-%m/"
        stac_date_url_all, date_id = url2date(
            URL0, DATE_FORMAT0, STR_FORMAT0, date_lim_tmp
        )
...
def url2month(URL, DATE_FORMAT, date_lim):
    ...
    # Detect url, id
    stac_date_url, date_id = detect_url_id(
        URL, DATE_FORMAT, year_lim, m_lim, m_lim_in, m_all
    )
...
def url2hmonth(URL, DATE_FORMAT, date_lim):
    ...
    hm_lim = [101, 1216]
    hm_lim_in = [
        date_lim[0].month * 100 + 15 * (date_lim[0].day // 16) + 1,
        date_lim[1].month * 100 + 15 * (date_lim[1].day // 16) + 1,
    ]

    # Set half month's number list
    hm_all = np.array([[1], [1]]) * np.linspace(1, 12, 12) * 100 + np.array(
        [[1], [16]]
    ) * np.ones([2, 12])
    ...
    # Detect url, id
    stac_date_url, date_id = detect_url_id(
        URL, DATE_FORMAT, year_lim, hm_lim, hm_lim_in, hm_all
    )
...
def url2doy(URL, DATE_FORMAT, date_lim):
    ...
    year_lim = [date_lim[0].year, date_lim[1].year]
    doy_lim = [1, 366]
    doy_lim_in = [date_lim[0].timetuple().tm_yday, date_lim[1].timetuple().tm_yday]
    doy_all = np.linspace(1, 366, 366)

    # Detect url, id
    stac_date_url, date_id = detect_url_id(
        URL, DATE_FORMAT, year_lim, doy_lim, doy_lim_in, doy_all
    )
...
def detect_url_id(URL, DATE_FORMAT, year_lim, date_lim, date_lim_in, date_all):
    ...
    for i in range(len(years)):
        ...
        if DATE_FORMAT == "DDD":
            date_lim[1] = (
                datetime.datetime(int(years[i]), 12, 31)
                - datetime.datetime(int(years[i]), 1, 1)
            ).days + 1
