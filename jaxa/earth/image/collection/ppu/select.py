...
def select_multiple_dates_ppu_url(input, ppu):
    ...
    for i in range(len(input.stac_date.url)):
        url_date_tmp = get_all_children(
            input._session,
            "child",
            input.stac_date.url[i][0],
            input.stac_date.json[i][0],
        )
        stac_ppu_url_tmp.append(
            select_ppu(url_date_tmp, user_ppu, ppu_max, proj_params)
        )
