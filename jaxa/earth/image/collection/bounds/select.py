...
def select_multiple_dates_bounds_url(input, bbox):
    ...
    for i in range(len(input.stac_ppu.url)):
        url_date_tmp1 = get_all_children(
            input._session, "child", input.stac_ppu.url[i][0], input.stac_ppu.json[i][0]
        )
        url_date_tmp2, lon_off_tmp = select_bounds(
            input._session, url_date_tmp1, bbox, input.proj_params
        )
