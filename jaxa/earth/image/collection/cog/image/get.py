...
def get_single_raster(session, fname, loff, geoj, ifd, roles, r_params, proj_params):
    ...
    tsize = np.array([ifd[-1]["TileWidth"][0], ifd[-1]["TileLength"][0]])
    imgsize = np.array([ifd[-1]["ImageLength"][0], ifd[-1]["ImageWidth"][0]])
    ...
    latlim, lonlim, latpix, lonpix, ppu = get_latlon_pos(
        ifd, geoj, imgsize, proj_params, loff
    )
    ...
    img = img[lat_pix_ROI[0] : lat_pix_ROI[1] + 1, lon_pix_ROI[0] : lon_pix_ROI[1] + 1]
