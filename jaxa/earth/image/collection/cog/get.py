...
def get_multiple_dates_raster(input):
    ...
    for i_date in range(len(fname)):
        ...
        raster_tmp, cinfo_tmp = get_multiple_bounds_raster(
            session, input, fn_date, loff_date
        )
...
def get_multiple_bounds_raster(session, input, fn_date, loff_date):
    ...
    if "classification:classes" in feat_cogs[0][0]["assets"][band]:
        ...
        value_info["lnames"] = {"name": "name", "value": "value", "color": "color"}
    ...
    # Calc image size
    img_size, pix_x, pix_y = calc_img_size(
        feat_cogs, feat_query, ifd_ppu, raster.ppu, pixels_max, proj_params, lon_offsets
    )
