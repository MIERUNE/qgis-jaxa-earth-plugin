...
def bin2img(bin, ifd, twidth, tlength):
    ...
    if ifd["Compression"][0] == 8 or ifd["Compression"][0] == 32946:
...
def bin2num0(bin, outtype):
    if outtype == "uint8":
        num = int.from_bytes(bin, byteorder="little", signed=False)
    elif outtype == "int8":
        num = int.from_bytes(bin, byteorder="little", signed=True)
    elif outtype == "uint16":
        num = int.from_bytes(bin, byteorder="little", signed=False)
    elif outtype == "int16":
        num = int.from_bytes(bin, byteorder="little", signed=True)
