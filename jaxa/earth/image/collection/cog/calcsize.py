...
# ----------------------------------------------------------------------------------------
def calc_img_size(
    feat_cogs, feat_query, ifd_ppu, img_ppu, PIX_NUM_MAX, proj_params, loffsets
):
...
def calc_all_latlon(feat_cogs, res, decimals, loffsets):
    ...
    for i in range(len(feat_cogs)):
        for j in range(len(feat_cogs[i])):
            ...
            bbox_tmp = [
                bbox_tmp[0] + loffsets[i][j],
                bbox_tmp[1],
                bbox_tmp[2] + loffsets[i][j],
                bbox_tmp[3],
            ]
    ...
    lat = np.round(
        np.linspace(latlim_all[1] - res / 2, latlim_all[0] + res / 2, latsize_all),
        decimals=decimals,
    )
    lon = np.round(
        np.linspace(lonlim_all[0] + res / 2, lonlim_all[1] - res / 2, lonsize_all),
        decimals=decimals,
    )
...
def check_img_size_rough(feat_query, ifd_ppu, PIX_NUM_MAX, proj_params):
    ...
    # Calc rough img size
    img_size = [
        round((lonlim_g[1] - lonlim_g[0]) / resolution),
...
def check_img_size(img_size, PIX_NUM_MAX):
    ...
    if pix_num > PIX_NUM_MAX:
        ...
        raise Exception(
            "Error! Requested ppd/LatLon is too large!\n" + message0 + message1
        )
