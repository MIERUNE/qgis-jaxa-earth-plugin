...
def mask_images(img, mask, type_query, values):
    ...
    if img_shape[1:3] != mask_shape[1:3]:
        ...
        raise Exception(
            "Error! image shape and mask shape is not match!\n" + message0 + message1
        )
    if (mask_shape[0] != 1) and (img_shape[0] != mask_shape[0]):
        message0 = f"img_shape : {  img_shape}\n"
        message1 = f"mask_shape : {mask_shape}"
        raise Exception(
            "Error! Mask shape time size must be 1 or same as image shape!\n"
            + message0
            + message1
        )
    if mask_shape[3] != 1:
    ...
    if type(values) != list:
        ...
        raise Exception(
            "Error! only list object will be allowed to input !\n" + message0
        )
...
def mask_bit(img, mask, values):
    # Check values
    bitmax = math.log2(np.iinfo(mask.dtype).max - np.iinfo(mask.dtype).min + 1)
    bitlen = len(values)
    if bitlen > bitmax:
