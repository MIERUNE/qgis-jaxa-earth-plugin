...
def match_images(data, mask, decimals):
    ...
    imgs_ovr_shape, imgs_ovr_lat_pix, imgs_ovr_lon_pix = extract(
        imgs, imgs_latlim, imgs_lonlim, latlim, lonlim, decimals
    )
    masks_ovr_shape, masks_ovr_lat_pix, masks_ovr_lon_pix = extract(
        masks, masks_latlim, masks_lonlim, latlim, lonlim, decimals
    )
    ...
    if imgs_ovr_shape == masks_ovr_shape:
        ...
        if masks[0].dtype == "float32":
            ...
            fillvalue = 0
        masks_new_shape = (
            tuple([np.array(masks).shape[0]]) + np.array(imgs).shape[1:3] + tuple([1])
        )
        ...
        for i in range(len(masks)):
            masks_tmp = masks[
                i,
                masks_ovr_lat_pix[0] : masks_ovr_lat_pix[1] + 1,
                masks_ovr_lon_pix[0] : masks_ovr_lon_pix[1] + 1,
            ]
            masks_new[
                i,
                imgs_ovr_lat_pix[0] : imgs_ovr_lat_pix[1] + 1,
                imgs_ovr_lon_pix[0] : imgs_ovr_lon_pix[1] + 1,
            ] = masks_tmp
...
def extract(imgs, imgs_latlim, imgs_lonlim, latlim, lonlim, decimals):
    ...
    imgs_ddeg = (
        np.array([imgs_latlim[1] - imgs_latlim[0], imgs_lonlim[1] - imgs_lonlim[0]])
        / imgs_shape[1:3]
    )
    imgs_lat = np.round(
        np.linspace(
            imgs_latlim[1] - imgs_ddeg[0] / 2,
            imgs_latlim[0] + imgs_ddeg[0] / 2,
            imgs_shape[1],
        ),
        decimals=decimals,
    )
    imgs_lon = np.round(
        np.linspace(
            imgs_lonlim[0] + imgs_ddeg[1] / 2,
            imgs_lonlim[1] - imgs_ddeg[1] / 2,
            imgs_shape[2],
        ),
        decimals=decimals,
    )
