...
def show_image(img, latlim, lonlim, title, cinfo, proj_params):
    ...
    lat_tick = np.linspace(
        latlim[-1] - lat_d / 2, latlim[0] + lat_d / 2, tick_itv
    ).round(decimals)
    lon_d = (lonlim[1] - lonlim[0]) / img.shape[1]
    lon_pix = np.linspace(0, img.shape[1] - 1, tick_itv)
    lon_tick = np.linspace(
        lonlim[0] + lon_d / 2, lonlim[1] - lon_d / 2, tick_itv
    ).round(decimals)
...
def show_image_qgis(id, date_id, img, latlim, lonlim, cinfo, proj):
    ...
    if raster is None:
        ...
        raise Exception(
            "Error! please check the inputed file path and permission to write/read!"
        )

    # Set Transform
    raster.SetGeoTransform((lonlim[0], lon_res, 0, latlim[1], 0, lat_res))
